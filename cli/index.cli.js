const js2flowchart = require("../chart-svg/js2flowchart.js");
...
const createAbstractedSvgFile = function (file, code, abstractionLevel) {
  ...
  let abstractions = abstractionLevel.map((al) => {
    try {
      ...
      throw new Error(
        `The following abstraction level isn't valid: ${al}\n${errMsg}`
      );
    }
  });
...
program
  ...
  .action(function (file) {
    if (program.abstraction)
      readJsFile(file, createAbstractedSvgFile, program.abstraction);
